...
--
-- | types for user command on Form Dialog
data CommandInfo = 
  CommandForm { _userCommand   :: ECU.UCommand   -- ^ ECUに発行するコマンド・バイト
              , _parameter     :: Maybe Int  -- ^ おそらく何らかの変更をする必要がある。
              }
--
--
-- # Brick related types
...
data Name  = StatusPane | DataPane | GraphPane | NotePane | ErrorContentsPane
   ...
   deriving (Eq,Ord,Show) -- MenuPane | StatusPane | CommandSelectPane deriving (Eq,Ord)
type Event = ECU.Event 
-- Menu
type Menu = String
-- type Menu  = [MenuItem]
testMenu :: String 
testMenu = "Q :Quit  | 1 :Initialize   | 2 :OffLine | d : Get dummy Data"
helpMenu :: String
helpMenu = "Q :Quit  | 0 :Clear Faults | 1 :Initialize | ← :Dec IAC Pos | → :Inc IAC POS | p :Get IAC Pos | ↑ :Inc IgAd | ↓ :Dec IgAd " :: [Char]
