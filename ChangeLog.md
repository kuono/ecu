...
- ロガースレッドの終了処理を若干修正（書き込みですぐフラッシュするようにし，未書き込みをなくす。）
...
- TUI ライブラリである brick の変化（1.0以降，型 BrickEvent が変わった）に対応するため，handleEventを書き直しした。

> ちなみに，BrickEvent は状態をモナドの文脈の奥に隠すように変更された。それ故，1.0 に満たない brick では handleEvent は明示的に新しい状態を返していたが，1.0 以降は get / put の組み合わせか，lens を使って状態変化を記述するようになった。

- Freeモナドを使ったeDSLによる制御に変更するための準備として，モジュールの分割を始め，関数や型定義を Main や Lib から Ecu や　UI に移動した（ただし，まだ main からは呼び出していないモジュールも多い。これらはとりあえずコンパイルが通る程度の記述であり，動作はしない）。

### 0.15.1 2022.07.17

- MEMSからのバイトデータもログに残すようにした
- Fault Codeを含むバイトデータを別途，画面表示するようにした
- プログラムの引数として，デバイスパスを指定できるようにした（ただし，Port Not Found のままプログラムを終了させるとSTM例外発生）
- ダミーデータで長さが正しくないデータも発生させるようにした（ただし，UI側で正しく処理できておらず，UI上では何も発生しない）
